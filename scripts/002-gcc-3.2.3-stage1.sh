#!/bin/sh
# gcc-stage1.sh 3.2.3 by uyjulian
# gcc-4.9.3 by AKuHAK

 ## Download the source code.
 OLD_GCC_VERSION=3.2.3
 SOURCE=http://ftpmirror.gnu.org/gcc/gcc-${OLD_GCC_VERSION}/gcc-${OLD_GCC_VERSION}.tar.bz2
 wget --continue $SOURCE || { exit 1; }

 ## Unpack the source code.
 echo Decompresing gcc ${OLD_GCC_VERSION}. Please wait.
 rm -Rf gcc-${OLD_GCC_VERSION} && tar xfj gcc-${OLD_GCC_VERSION}.tar.bz2 || { exit 1; }

 ## Enter the source directory and patch the source code.
 cd gcc-${OLD_GCC_VERSION} && cat ../../patches/gcc-${OLD_GCC_VERSION}-PS2.patch | patch -p1 || { exit 1; }
 ...
 ## Unpack the source code.
 echo Decompresing gcc ${GCC_VERSION}. Please wait.
 rm -Rf gcc-${GCC_VERSION} && tar xfj gcc-${GCC_VERSION}.tar.bz2|| { exit 1; }

 ## Enter the source directory and patch the source code.
 cd "gcc-${GCC_VERSION}" && cat ../../patches/gcc-${GCC_VERSION}-mips-PS2.patch | patch -p1 || { exit 1; }
